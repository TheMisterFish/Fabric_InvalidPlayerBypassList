	...
	private void registerCommands(CommandDispatcher<ServerCommandSource> dispatcher) {
		dispatcher.register(CommandManager.literal("bypasslist")
						...
						.then(CommandManager.argument("player", StringArgumentType.word())
								.executes(ctx -> {
									...
									if (added) {
										...
										ctx.getSource().sendFeedback(() -> Text.literal(player + " with IP none is already on the bypass list."), false);
									}
									return 1;
								})
								.then(CommandManager.argument("ip", StringArgumentType.word())
										.executes(ctx -> {
											...
											if (added) {
												...
												ctx.getSource().sendFeedback(() -> Text.literal(player + " with IP " + ip + " is already on the bypass list."), false);
											}
											return 1;
										})
								)
				...
				.then(CommandManager.literal("on")
						.executes(ctx -> {
							bypassList = true;
							ctx.getSource().sendFeedback(() -> Text.literal("Invalid player bypass list enabled."), false);
							return 1;
						})
				)
				...
				.then(CommandManager.literal("off")
						.executes(ctx -> {
							bypassList = false;
							ctx.getSource().sendFeedback(() -> Text.literal("Invalid player bypass list disabled."), false);
							return 1;
						})
				)
		);
